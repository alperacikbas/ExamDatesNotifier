...
class ExamCreateViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        hoursStepper.maximumValue = 72
        hoursStepper.stepValue = 1
        hoursStepper.value = 1
        hoursLabel.text = "1 saat"
    }
    ...
    @IBAction func saveButtonTapped(_ sender: UIButton){
        guard let examName = examNameTextField.text, !examName.isEmpty else {
            showAlert(title: "Hata", message: "Lütfen tüm alanları doğru doldurun.")
            return
        }
        
        let examDate = datePicker.date
        let hoursBefore = hoursStepper.value
